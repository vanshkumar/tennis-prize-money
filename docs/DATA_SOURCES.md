...
Version `0.1.0` shipped a small seed dataset for 2025 Grand Slam men's singles prize money. The current unreleased dataset adds schema-version-2 numerator semantics, one US Open total-player-compensation context row, and the first compatible primary-question Wimbledon tournament-total financial slice. The active dataset is stored in:
...
The active seed includes one compatible financial denominator slice:
...
The filing also reports net finance income, a £48.117m division of net available surplus to LTA Operations, £6.737m profit before tax, £5.591m profit after tax, and a £5.0m dividend. Those values are not used as the primary denominator. They remain caveats because they reflect surplus division, retained profit, or distribution mechanics rather than the operating profit denominator selected for the first Wimbledon row.

All non-Wimbledon seed records still leave tournament-level revenue, profit, or surplus unavailable. USTA organization-level financials, other organization-level revenues, press estimates, and player-share claims are not normalized as compatible denominators unless a future task can tie them to a specific tournament/event and currency.
...
| --- | --- | --- | --- | --- |
| 1 | Wimbledon | Normalized 2025 clean competition-prize row uses £52.0m total tennis events prize money. The broader £53.5m total prize money line includes £1.5m estimated per diems and is not used as the clean numerator. | Normalized 2025 denominators: AELTC Championships Ltd turnover £423.626m and operating profit £52.720m. | Completed for 2025 with explicit operating-company caveats. Possible next Wimbledon work: add 2024 prior-year tournament-total row for YoY comparisons. |
...
- Revenue and profit/surplus are available only for the Wimbledon tournament-total row; other rows remain unavailable, not estimated or inferred.
